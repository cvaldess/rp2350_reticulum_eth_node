 ...
 *
 * Modified for arduino-pico (RP2040/RP2350) from microStore's
 * include/microStore/Adapters/LittleFSFileSystem.h, which targets the ESP32
 * LittleFS API: begin(bool, path) -> begin(), totalBytes()/usedBytes() -> info(),
 * FILE_* mode macros -> string literals. Candidate for an upstream PR.
 */
...
#include <microStore/Log.h>

#include <LittleFS.h>

namespace microStore { namespace Adapters {

class PicoLittleFSFileSystem : public microStore::FileSystem {

public:
	PicoLittleFSFileSystem() : microStore::FileSystem(new FileSystemImpl()) {}
	virtual ~PicoLittleFSFileSystem() {}
...
protected:
	...
	public:
		...
		inline virtual bool format() override {
			USTORE_LOG("[ustore] Formatting PicoLittleFSFileSystem\n");
			if (!LittleFS.format()) {
				USTORE_LOG("[ustore] Failed to format PicoLittleFSFileSystem!\n");
				return false;
			}
			return true;
		}
		...
		inline virtual bool init(bool reformatOnFail = true) override {
			USTORE_LOG("[ustore] Initializing PicoLittleFSFileSystem\n");
			// arduino-pico: begin() auto-formats an unformatted partition (LittleFSConfig autoFormat=true)
			if (!LittleFS.begin()) {
				USTORE_LOG("[ustore] Failed to initialize PicoLittleFSFileSystem!\n");
				return false;
			}
			if (reformatOnFail) {
				...
				if (!verified) {
					USTORE_LOG("[ustore] WARNING: PicoLittleFSFileSystem check failed, reformatting!\n");
					format();
				}
				else {
					remove("/__init_test__");
					USTORE_LOG("[ustore] PicoLittleFSFileSystem check passed!\n");
				}
			}
			return true;
		}
		...
		inline virtual size_t storageAvailable() override {
			fs::FSInfo info;
			if (!LittleFS.info(info)) return 0;
			return (size_t)(info.totalBytes - info.usedBytes);
		}
	};
